...
void CudaRtHandler::RegisterFatBinary(std::string& handler, void ** fatCubinHandle) {
    ...
    cout << "Registered FatBinary " << fatCubinHandle << " with handler " << handler << endl;//这里是正确执行时候的显示
}
...
//Sandy test04 就是这里是出错提示的地方，这个函数由下面这个函数调用
void ** CudaRtHandler::GetFatBinary(string & handler) {
    map<string, void **>::iterator it = mpFatBinary->find(handler);
    if (it == mpFatBinary->end())
        throw "Fat Binary '" + handler + "' not found";		//错误点就在这里  2016.04.05
    return it->second;
}
...
void CudaRtHandler::UnregisterFatBinary(std::string& handler) {
    ...
    cout << "Unregistered FatBinary " << it->second << " with handler "//这个执行错误病没有被抛出，那么原因只能是上一个return语句起作用了
            << handler << endl;
    mpFatBinary->erase(it);
}
